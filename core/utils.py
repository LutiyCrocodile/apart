    ...
    for response_json in cian_parser_data:
        ...
        for key, value in field_name.items():
            async with connection() as session:
                fetch_query = text(
                    f"SELECT {value} FROM real_estate.{key} WHERE {key} = :value"
                )
                result = await session.execute(
                    fetch_query, {"value": str(response_json.get(key))}
                )
                existing_record = result.fetchone()

                if not existing_record:
                    insert_query = text(
                        f"INSERT INTO real_estate.{key} ({key}) VALUES (:value) RETURNING {value}"
                    )
                    inserted_value = await session.execute(
                        insert_query, {"value": str(response_json.get(key))}
                    )
...
def get_cian_id(url: str) -> int | None:
